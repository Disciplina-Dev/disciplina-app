...
| ID | Titre | Constat | Fichiers | Action | Done quand | Taille |
|---|---|---|---|---|---|---|
| DB-4 | Comptes de moindre privilège | **MySQL fait** : l'app se connecte en `disciplina_app` (`database/mysql/mysql-init.sql`), sans `DROP` ni privilège global — `DROP DATABASE`, `DROP TABLE` et `mysql.user` refusés, vérifié ; la suite de tests tourne sous ce compte, donc une requête réclamant un privilège de trop échoue en CI. `CREATE`/`ALTER` conservés (migrations au boot) et `CREATE TEMPORARY TABLES` (CTE des KPI). Bases existantes : le compte a été créé par la migration one-off de 2026-08 (dossier `database/mysql/migrations/` supprimé depuis) ; fallback `MYSQL_USER=root`. **Reste ouvert** : (1) Mongo utilise toujours le compte root initdb ; (2) pas de compte lecture seule pour KPI/analytics ; (3) prod TiDB (`MYSQL_URI`) à aligner côté TiDB Cloud | `database/mysql/mysql-init.sql`, `docker-compose*.yaml`, `back/src/config/env.ts` | Créer un user applicatif Mongo + un user lecture seule KPI ; appliquer les mêmes GRANT sur TiDB | Aucune des trois bases (MySQL local, Mongo, TiDB) n'est atteinte avec des identifiants root par l'app | S |
| DB-5 | Backups automatisés | Dump quotidien MySQL + Mongo automatisé via **launchd** sur le Mac mini de prod (`scripts/backup-db.sh`, `scripts/launchd/com.disciplina.backup.plist`), rétention glissante 7 jours locale, restauration testée manuellement (cf. `README.md#backups`). **Reste ouvert** : stockage hors machine (les dumps ne quittent jamais le Mac mini — perte du disque = perte des backups) | `scripts/backup-db.sh`, `scripts/launchd/com.disciplina.backup.plist` | Répliquer `backups/` vers un stockage distant (rsync/rclone/S3-compatible) après chaque run | Un backup est récupérable même si le Mac mini est hors service | S |
...
| DB-7 | « Diviser les tables en différentes bases » — requalification | **Fait.** Découpage en 5 schémas étudié puis écarté : le gain visé était l'isolation par droits, or `KpiRepository.ts` et `RhKpiRepository.ts` joignent `users` avec `companies`/`contact_logs`/`company_history`, donc un compte par schéma casse ces requêtes et un compte unique sur 5 schémas réduit le découpage à un renommage. Objectif requalifié en cloisonnement par privilège (= DB-4, MySQL fait) ; classification RGPD par table dans `database/DATA_CLASSIFICATION.md`, décision tracée en §5 du même document | `database/DATA_CLASSIFICATION.md` | — | Décision écrite et validée ✅ | XS |
