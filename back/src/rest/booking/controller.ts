...
function parseWorkingHours(raw: unknown): WorkingHours | null {
    ...
    for (const [day, windows] of Object.entries(raw as Record<string, unknown>)) {
        ...
        for (const w of windows) {
            if (!Array.isArray(w) || w.length !== 2 || !HM_RE.test(w[0]) || !HM_RE.test(w[1]) || w[0] >= w[1]) return null;
            parsed.push([w[0], w[1]]);
        }
        out[day] = parsed;
    }
...
export async function updateMySettings(req: AuthRequest, res: Response): Promise<void> {
    ...
    if (b.durationMin !== undefined) {
        ...
        if (v === undefined) { res.status(400).json({ error: 'durationMin invalide (5–480)' }); return; }
        patch.durationMin = v;
    }
    if (b.bufferMin !== undefined) {
        const v = clampInt(b.bufferMin, 0, 240);
        if (v === undefined) { res.status(400).json({ error: 'bufferMin invalide (0–240)' }); return; }
        patch.bufferMin = v;
    }
    if (b.minNoticeHours !== undefined) {
        const v = clampInt(b.minNoticeHours, 0, 720);
        if (v === undefined) { res.status(400).json({ error: 'minNoticeHours invalide (0–720)' }); return; }
        patch.minNoticeHours = v;
    }
    if (b.maxDaysAhead !== undefined) {
        const v = clampInt(b.maxDaysAhead, 1, 365);
        if (v === undefined) { res.status(400).json({ error: 'maxDaysAhead invalide (1–365)' }); return; }
        patch.maxDaysAhead = v;
    }
    if (b.timezone !== undefined) {
        if (typeof b.timezone !== 'string' || !isValidTimezone(b.timezone)) {
            res.status(400).json({ error: 'timezone invalide' }); return;
        }
        patch.timezone = b.timezone;
    }
    if (b.workingHours !== undefined) {
        const wh = parseWorkingHours(b.workingHours);
        if (!wh) { res.status(400).json({ error: 'workingHours invalide' }); return; }
        patch.workingHours = wh;
    }
    if (b.title !== undefined) {
        const t = String(b.title).trim();
        if (!t || t.length > 255) { res.status(400).json({ error: 'title invalide' }); return; }
        patch.title = t;
    }
...
async function publicView(slug: string) {
    ...
    if (!host) return null;
    return { settings, hostName: host.name };
}
...
export async function getPublicBooking(req: Request, res: Response): Promise<void> {
    ...
    if (!view) { res.status(404).json({ error: 'Page de réservation introuvable ou désactivée' }); return; }
    res.json({
...
export async function getPublicSlots(req: Request, res: Response): Promise<void> {
    ...
    if (typeof from !== 'string' || typeof to !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(from) || !/^\d{4}-\d{2}-\d{2}$/.test(to)) {
        res.status(400).json({ error: 'from et to (YYYY-MM-DD) requis' }); return;
    }
    const settings = await bookingService.findBySlug(req.params.slug);
    if (!settings || !settings.enabled) { res.status(404).json({ error: 'Réservation indisponible' }); return; }
    try {
        res.json({ slots: await bookingService.computeSlots(settings, from, to), durationMin: settings.durationMin, timezone: settings.timezone });
    } catch (err) {
        if (err instanceof BookingDisabledError) { res.status(409).json({ error: err.message }); return; }
...
export async function postBooking(req: Request, res: Response): Promise<void> {
    ...
    if (!start || !name || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
        res.status(400).json({ error: 'start, name et email valides requis' }); return;
    }
    const settings = await bookingService.findBySlug(req.params.slug);
    if (!settings || !settings.enabled) { res.status(404).json({ error: 'Réservation indisponible' }); return; }
    try {
        ...
        if (err instanceof SlotUnavailableError) { res.status(409).json({ error: err.message }); return; }
        if (err instanceof BookingDisabledError) { res.status(409).json({ error: err.message }); return; }
