...
1. Load and run the `verify-backend` skill and the `verify-frontend` skill (lint + build + backend tests, lint + build for front). Note pass/fail for each step.
2. In parallel, launch these 4 subagents against `git diff main...HEAD`:
   - `security-reviewer`
   - `convention-checker`
...
Keep the final summary short: a bullet list, not a report. Don't re-run the agents' underlying checks yourself — trust their findings output, but sanity-check any CONFIRMED finding against the actual file before listing it as a blocker.
