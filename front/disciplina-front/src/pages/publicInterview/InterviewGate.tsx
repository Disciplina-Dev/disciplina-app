...
import { inspectInterview, type InterviewInspectResult } from '@/api/interview'
import InterviewGateForm from '@/features/publicInterview/components/InterviewGateForm'
...
export default function InterviewGate() {
  ...
  const [inspect, setInspect] = useState<InterviewInspectResult | null>(null)
  const [loadError, setLoadError] = useState<string | null>(null)
  const [locked, setLocked] = useState(false)

  useEffect(() => {
    if (!signature) return
    inspectInterview(signature)
      .then(setInspect)
  ...
  return (
    <Centered>
      <div className="w-full max-w-sm rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-1 text-[20px] font-extrabold text-gray-900">Créneau d'entretien</h1>
        ...
        {blocked ? (
          ...
          <div className="mt-5">
            <p className="mb-3 text-[13px] text-gray-500">
              Saisissez le code reçu par email pour choisir votre créneau d'entretien.
            </p>
            <InterviewGateForm signature={signature} onLocked={() => setLocked(true)} />
          </div>
        )}
      </div>
    </Centered>
