...
import { saveKpi, KPI_SITES, type KpiMetrics, type KpiSite } from '@/api/kpi'
...
export default function KpiEntryModal({ year, site, draft, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError(null)
    try {
      await saveKpi(token, { user_name: userName.trim(), year, month, week, site: entrySite, ...metrics })
      onSaved()
      onClose()
    } catch (err) {
