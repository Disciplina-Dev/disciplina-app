...
import { authenticateCvImport } from '@/api/cvImport'
...
export default function CvImportGateForm({ signature, onLocked }: { signature: string; onLocked: () => void }) {
  const navigate = useNavigate()
  const setToken = useGuestCvImportTokenStore((s) => s.setToken)
  ...
  const submit = async () => {
    ...
    setBusy(true)
    try {
      const result = await authenticateCvImport(signature, code.trim())
      if (result.ok) {
        setToken(result.token)
        navigate(`/public/cv-import/${signature}`)
        return
      }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        {busy ? <Loader2 size={16} className="animate-spin" /> : <LogIn size={16} />} Accéder à l'import
      </button>
    </div>
  )
}
