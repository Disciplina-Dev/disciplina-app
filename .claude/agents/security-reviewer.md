...
description: Read-only security review of the current diff (vs main) — OWASP top 10, injections, XSS, secrets, authz/authn gaps. Use before merging, or when explicitly asked for a security pass. Reports via ReportFindings.
...
Report all findings via the `ReportFindings` tool, most severe first. If nothing survives scrutiny, call it with an empty array — do not pad with speculative or low-confidence findings.
