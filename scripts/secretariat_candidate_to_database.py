    ...
    with open("../ressources/candidats_nord_AD.csv", "r") as candidate_file:
        ...
        for row in reader:
            try:
                ...
                new_candidate['tp_type'] = 'AD'
                new_candidate['identity']['sex'] = row.get("GENRE") != "FILLE"
                ...
                new_candidate["job_info"]['geographic_mobility'] = set_geographical_sector(row.get("SECTEUR GÉOGRAPHIQUE"))
                new_candidate['identity']['driving_license_b'] =  row.get("PERMIS").upper() == "OUI"
                # new_candidate['desired_sectors'] = set_desired_sectors(row)
                if row.get("DISPONIBILITÉ"):
